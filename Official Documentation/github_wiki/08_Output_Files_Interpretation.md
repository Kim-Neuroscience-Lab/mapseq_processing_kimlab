...
This chapter lists the main files and directories produced by the MAPseq processing pipeline and helper scripts: **locations, names, and column semantics**. Biological or study-specific interpretation depends on your design, thresholds, and model choice; use [Chapter 5: Statistical Methods](05_Statistical_Methods) for definitions of tests and effect sizes, and [Chapter 11: Stability Analysis](11_Stability_Analysis) for a generic framework (without project-specific verdicts).

Optional batch utilities (figure aggregation, conclusions generation) are described in [Chapter 14: Experimental Features](14_Experimental_Features).
...
Additional model subdirectories under `analysis/` may appear when using extended `--model-type` values (see [Chapter 4](04_Main_Processing_Pipeline)).
...
**Key outputs**: Kruskal-Wallis and related tables, JSD tables, plots (see script and [Chapter 7](07_Helper_Scripts)).
...
**Missing helper outputs**: Wrong execution order or missing inputs; see [Chapter 7](07_Helper_Scripts).

**QC script**: `postprocessing_checks.py` can summarize run health; run from repo root with `--help` for arguments.

---

*For troubleshooting, see [Chapter 12: Troubleshooting and Best Practices](12_Troubleshooting_Best_Practices). Maintainer-only batch tools are in [Chapter 14: Experimental Features](14_Experimental_Features).*
