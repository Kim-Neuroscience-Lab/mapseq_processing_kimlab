...
The repository’s [`all_commands.txt`](../../all_commands.txt) is the reference ordering for this project. A typical block (after all `process-nbcm-tsv.py` runs) is:
...
**Status**: Optional visualization; appears in the repository `all_commands.txt` as the script named `10_...` (not the dataset-compare script below).
