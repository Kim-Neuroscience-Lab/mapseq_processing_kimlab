...
def calculate_projections_from_matrix(matrix, sample_labels):
    """Calculate projections from the matrix for each label."""
...
def clean_and_filter(matrix, sample_labels):
    ...
    if not neg_columns:
        print("No columns containing 'neg' found. Skipping Step 5.")
    else:
...
filtered_matrix = clean_and_filter(barcodematrix, sample_labels)
...
# Dynamically find indices of all "neg" columns
neg_columns = [i for i, label in enumerate(sample_labels) if "neg" in label.lower()]

if neg_columns:
    # Drop all "neg" columns
    normalized_matrix = np.delete(normalized_matrix, neg_columns, axis=1)
    print(f"Dropped 'neg' columns at indices: {neg_columns}.")
else:
    print("No columns containing 'neg' found. Skipping column removal.")

# Filter out labels corresponding to dropped "neg" columns
columns = [label for i, label in enumerate(sample_labels) if i not in neg_columns]

# Save the normalized matrix to CSV with accurate headers
...
projections = calculate_projections_from_matrix(filtered_matrix, sample_labels)
